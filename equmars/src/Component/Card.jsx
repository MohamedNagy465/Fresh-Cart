...
export default function Card({ item, type }) {
  ...
  function handleClick() {
    ...
    else if (type === "category") navigate(`/CatogeryDatels/${item._id}`);
  }
  ...
  return (
    ...
    >
      <div className="relative group overflow-hidden">
        ...
        <div className="icon flex gap-2 absolute bottom-[-40px] left-1/2 -translate-x-1/2 group-hover:bottom-[50%] transition-all duration-300">
          <Heart
            onClick={(e) => {
              e.stopPropagation();
              addToWishList(item?._id);
              setFlag(!flag); // لتغيير اللون لو حبيت تضيف تفاعل
            }}
            className={`bg-green-500 p-1 rounded-full shadow-md cursor-pointer ${
              flag ? "text-red-500" : "text-white"
            }`}
          />
